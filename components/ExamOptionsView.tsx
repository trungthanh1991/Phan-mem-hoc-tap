...
const ExamOptionsView: React.FC = () => {
    const { selectedSubject, handleSelectExamDuration, handleBackToTopicSelection } = useGame();

    if (!selectedSubject) return null;

    return (
        <div className="w-full max-w-2xl mx-auto p-4 md:p-6 text-center animate-fade-in-up">
            <button onClick={handleBackToTopicSelection} className="text-primary hover:underline mb-6">&larr; Quay lại</button>
